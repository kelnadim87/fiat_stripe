class FiatStripe::Invoice::SendNoticeJob < FiatStripe::ApplicationJob
  ...
  def perform(invoice, email_recipients, invoice_url)
    ...
    if invoice.sent? && !invoice.sent_date
      ...
      email_template_id = FiatStripe.configuration.invoice_notice_email_template_id
    elsif invoice.received? && !invoice.received_date
      invoice.update(received_date: Date.today)
      email_template_id = FiatStripe.configuration.invoice_receipt_email_template_id
    end
    ...
    postmark_client = Postmark::ApiClient.new(FiatStripe.configuration.postmark_api_token)
    postmark_client.deliver_with_template(
    {:from=>FiatStripe.configuration.from_email_address,
     :to=>email_recipients,
